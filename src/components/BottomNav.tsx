...
const NAV_ITEMS = [
  ...
  { path: '/add-info', icon: Plus, labelKey: 'add', isCenter: true },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[#0a1628]/95 backdrop-blur-md border-t border-white/10">
      <div className="flex items-center justify-around max-w-md mx-auto px-2 py-2">
        {NAV_ITEMS.map(({ path, icon: Icon, labelKey, isCenter }) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 transition-colors ${active ? 'text-cyan-400' : 'text-gray-500'}`} />
              <span className={`text-xs transition-colors ${active ? 'text-cyan-400' : 'text-gray-500'}`}>
                {t(labelKey)}
              </span>
            </button>
          );
